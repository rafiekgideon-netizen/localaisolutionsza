...
const getIndustryImageResponsiveProps = (id: string, altText: string) => {
  const baseUrl = `${UNSPLASH_BASE_URL}/${id}`;
  return {
...
function IndustryCard({ name, icon: Icon, imgId, onClick, compact = false }: IndustryCardProps) {
  const imageProps = getIndustryImageResponsiveProps(imgId, name);
  return (
    <button
      type="button"
      onClick={onClick}
      aria-label={`Start Strategic Revenue Audit for ${name}`}
      className={`bg-[#111] border border-[rgba(255,255,255,0.06)] group hover:border-[var(--color-tertiary)] hover:-translate-y-1 transition-all duration-300 cursor-pointer flex flex-col justify-between relative overflow-hidden text-left ${
        compact ? "p-5 min-h-[140px]" : "flex-shrink-0 w-[240px] p-6 min-h-[160px]"
      }`}
    >
      <img 
        {...imageProps}
      ...
      <div className="relative z-10">
        <div className={`${compact ? "mb-3" : "mb-4"} text-[var(--color-tertiary)] opacity-80 group-hover:opacity-100 group-hover:-translate-y-1 transition-all duration-300 relative`}>
          ...
          <Icon className={`${compact ? "w-7 h-7" : "w-8 h-8"} relative z-10 pulse-on-hover`} strokeWidth={1.5} />
        </div>
        <h3 className={`font-display ${compact ? "text-lg" : "text-xl"} text-white m-0 tracking-tight`}>
          {name}
        </h3>
      </div>

      <div className={`mt-4 ${compact ? "pt-3" : "pt-4"} border-t border-[rgba(255,255,255,0.03)] opacity-80 sm:opacity-0 sm:translate-y-1 group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-300 flex items-center justify-between relative z-10`}>
        <span className={`font-mono ${compact ? "text-[9px]" : "text-[10px]"} text-[var(--color-tertiary)] uppercase tracking-widest inline-flex items-center gap-1.5 font-bold`}>
          Learn how <ArrowRight className={`${compact ? "w-3 h-3" : "w-3.5 h-3.5"} group-hover:translate-x-1 transition-transform`} />
        </span>
      </div>
    </button>
  );
