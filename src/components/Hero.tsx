...
export function Hero({ onOpenAudit }: HeroProps) {
  ...
  useSpring(mouseX, { stiffness: 40, damping: 25, mass: 1.5 });
  useSpring(mouseY, { stiffness: 40, damping: 25, mass: 1.5 });
  ...
  return (
    <section 
      id="home" 
      ref={heroRef}
      aria-label="Introduction and Strategic Revenue Overview"
      className="hero-section min-h-[100dvh] flex items-center pt-32 pb-24 relative overflow-hidden"
    >
      {/* Background Grainient */}
      <div 
        className="absolute inset-0 overflow-hidden z-0 pointer-events-auto bg-[#050505]"
      ...
      >
        <div 
          className="absolute inset-0 w-full max-w-full h-[100dvh] opacity-60 hover:opacity-80 hover:brightness-110 transition-all duration-700 ease-out cursor-default"
          style={{ 
